...
struct ApiWebSocketTests {
    ...
    func dataCommuteTest(paras: (Int, String, Int))  {
        ...
        Task {
            do {
                try await ws.connect(to: "ws://localhost:\(TestingShared.apiListenPort)/websocket-echo-\(suffix)", configuration: .init(maxFrameSize: Int(UInt32.max))) { ws in
                    Task {
                        var printIndex = 0
                        for i in 0..<times {
    ...
    func connectionErrorTest() async throws {
        await #expect(throws: ApiWebSocket.Failure.self, performing: { try await ws.connect(to: "ws://127.0.0.1:100000", onUpgrade: { _ in }) })
    }
