...
@Suite("Api 基本网络通讯测试集", .serialized)
struct ApiNormalTests {
    ...
    func start() async throws {
        while await TestingShared.testStage != .apiNormal {
    ...
    func sendRequestTest(method: HTTPMethod) async throws {
        let res = try await client.send(method, to: "http://localhost:\(TestingShared.apiListenPort)/string-echo?value=\(testString)", body: .text(testString).get())
        #expect(res.status == .ok)
        #expect(res.headers.contains(name: "content-length"))
        let length = try Int(#require(res.headers.first(name: "content-length")))
