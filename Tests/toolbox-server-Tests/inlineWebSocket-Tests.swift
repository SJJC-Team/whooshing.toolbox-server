...
struct InlineWebSocketTests {
    ...
    func start() async throws {
        while await TestingShared.testStage != .inlineWebSocket {
    ...
    func dataCommuteTest(paras: (Int, String, Int))  {
        ...
        Task {
            do {
                try await ws.connect(to: "ws://localhost:\(TestingShared.inlineListenPort)/websocket-echo-\(suffix)", configuration: .init(maxFrameSize: Int(UInt32.max))) { ws in
                    Task {
                        var printIndex = 0
                        for i in 0..<times {
