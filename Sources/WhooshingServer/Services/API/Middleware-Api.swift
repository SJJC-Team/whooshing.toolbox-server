            ...
            return req.eventLoop.submitResult { () throws(Failure) in
                ...
                case .target(.itself):
                    ...
                    return req.eventLoop.submitResult { () throws(Failure) in
                        ...
                        try required(throws: Errcase.jsonEncodeFailed, "将认证请求信息写入请求体失败", category: .internal) {
                            try request.content.encode(authData, as: .json)     // 准备要用来发送请求的 body
                        }
                        
                        return request
                    }.flatMap { request in
                        req.logger.debug("向本机服务发送认证请求", metadata: ["request": .data(request.description)])
                        return httpsServer.app.responder.respond(to: request)
                ...
                case .target(.url(let url)):
                    req.logger.debug("API.Server-与客户端密钥交换: 向认证模块发送认证请求")
                    return req.eventLoop.submitResult { () throws(Failure) in
                        try required(throws: Errcase.jsonEncodeFailed, "将认证请求信息写入请求体失败", category: .inherit) {
                            try HTTPBody.json(authData).get()   // 准备要用来发送请求的 body
                        }
                    }.flatMap { body in
                        req.application.apiServiceData.inlineClient.post(
                            url.toUri(with: AuthenticationTarget.selfURL.string),
                            body: body
                        )
                    ...
                    .flatMapThrowing { res throws(Failure) in
                        ...
                        let buffer = try required(throws: Errcase.internalFailure, "未成功从响应体解析 ByteBuffer", category: .internal) {
